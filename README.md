...
- Для фронтенда используется React ([Create React App](https://github.com/facebook/create-react-app)).
...
Фронтенд также должен работать одновременно с бэкендом,
поэтому для его запуска удобно будет создать отдельный
экземпляр командной строки.
...
При первом запуске (только один раз) выполняем сборку:
```
npm install
```

Далее запускаем сам фронтенд:
```
npm start
```

TODO: перенести в этот раздел (про фронтенд) инструкцию использования
из части про бэкенд
